...
function Starred() {
    ...
    const handleStar = async (fileId:string)=>{
        try{
            ...
            const filterFunction = (key:string)=>{
                const keys :string[]= ['http://localhost:3000/api/files/','http://localhost:3000/api/files/starred/']
                return keys.includes(key)
            }
            await mutate(filterFunction)
            // mutate('http://localhost:3000/api/files/')
            // mutate('http://localhost:3000/api/files/starred/')
    ...
    const handleTrash = async (fileId:string)=>{
        ...
        try{
            ...
            const filterFunction = (key:string)=>{
                const keys :string[]= ['http://localhost:3000/api/files/','http://localhost:3000/api/files/starred/','http://localhost:3000/api/files/trash/']
                return keys.includes(key)
            }
            await mutate(filterFunction)
    ...
    <div className="-2 w-full rounded-md h-[98%] border-1 border-gray-300 flex flex-col gap-4 p-3 text-white my-[2%] self-center">
        <div className={isLoading?`absolute w-full h-full bg-black/10 top-0 left-0 rounded-md `:`hidden`} onClick={e=>e.stopPropagation()} onDoubleClick={e=>e.stopPropagation()}>
